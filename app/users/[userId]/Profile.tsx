...
export const Profile = ({user, children}:PropsWithChildren<{user:UserProfile}>) => {
  return (
    <div className='mt-4 container'>
        ...
        {user.bio ? (<p>{user.bio}</p>): <p className='text-muted-foreground'>no bio</p>}
        <div className='flex items-center gap-2 mt-4'>
            <div className='flex -space-x-2'>
                {user.followeds.map(foll => ( <Avatar key={foll.follower.id} size='sm' className='border-2 border-background'>
                {foll.follower.image ? <AvatarImage src={foll.follower.image} alt={foll.follower.username}/> : null}
                <AvatarFallback>
                    {foll.follower.username.slice(0, 3).toUpperCase()}
                </AvatarFallback>
            </Avatar>))}
